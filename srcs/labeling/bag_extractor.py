...
for filename, bagname in zip(filenames, bagnames):
    ...
    for topic, msg, t in data:
        if topic == pointcloud_topic:
            ...
            # Create a directory named the same as
            # the bag in which to save the data
            pcl_dir = os.path.join(save_dir, filename)
            mkdir_p(pcl_dir)

            # Save the point cloud with the name as
            # the timestamp of this message
            np.savez(os.path.join(pcl_dir, str(timestamp)),
                     pointcloud=pc, timestamp=timestamp_arr)
            num_pcl += 1
