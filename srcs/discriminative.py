...
def calculate_means(pred, gt, n_objects, max_n_objects, usegpu):
    ...
    for i in range(bs):
        ...
        # TODO check if _n_objects_sample is zero here
...
def calculate_variance_term(pred, gt, means, n_objects, delta_v, norm=2):
    ...
    var_term = 0.0
    for i in range(bs):
        # TODO check if n_objects[i] is zero here
        if n_objects[i] != 0:
...
def calculate_regularization_term(means, n_objects, norm):
    ...
    reg_term = 0.0
    for i in range(bs):
        # TODO check if n_objects is zero here
        if n_objects[i] != 0:
...
#         super(DiscriminativeLoss, self).__init__(size_average)
    def __init__(self, delta_var, delta_dist, norm,
                 reduction='mean', usegpu=True):
        super(DiscriminativeLoss, self).__init__(reduction)
